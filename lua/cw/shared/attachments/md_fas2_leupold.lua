...
if CLIENT then
	...
	local scopeLeo = surface.GetTextureID("models/qq_rec/fas2_2015/scope_leo")
	...
	function att:drawRenderTarget()
		...
		local correctMDL = self.AttachmentModelsVM[att.name] and self.AttachmentModelsVM[att.name].model == "models/v_fas2_leupold.mdl"
		local isScopePos = (self.AimPos == self[att.aimPos[1]] and self.AimAng == self[att.aimPos[2]])
		
		if (isAiming /*or freeze*/) and correctMDL and isScopePos then 
			self.AttachmentModelsVM[att.name].ent:SetBodygroup(1, 1)
		elseif correctMDL then 
			self.AttachmentModelsVM[att.name].ent:SetBodygroup(1, 0)
		end
		...
		if not complexTelescopics then
			...
			cam.Start2D()
				...
				if not self.AttachmentModelsVM[att.name].disableRF then
					local trace = {}
